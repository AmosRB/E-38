...
setInterval(async () => {
  ...
  // צלחות מעופפות יורות על TAKILA (1 ק״מ)
  landings.forEach(l => {
    takilas.forEach(t => {
  ...
  // מחיקת landings אחרי 4 פגיעות ומחיקת החייזרים שלהם
...
app.post('/api/create-landing', async (req, res) => {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    aliens.push({ id: Date.now() + i, lat: aRoute[0][0], lng: aRoute[0][1], hitCount: 0, route: aRoute, positionIdx: 0, landingId: id });  // ✅ הוספנו landingId
  }
